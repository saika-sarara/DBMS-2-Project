...
BEGIN
    ...
    WITH picked AS (
        ...
        FROM public.fn_quiz_pick_questions(v_quiz_id, p_count)
    )
    SELECT picked.question_id, qq.question_text,
           qo.id, qo.option_label, qo.option_text
    FROM picked
    JOIN public.quiz_questions qq ON qq.id = picked.question_id
